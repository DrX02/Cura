...
class RescheduleForm extends Component
{
    ...
    public function updateAppointment($slot){
        ...
        $appointmentEmailData = [
            ...
            'appointment_type' => $this->appointment_details->appointment_type == 0 ? 'sur place' : 'consultation en direct',
        ...
        session()->flash('message','Rendez-vous avec'.$this->doctor_details->doctorUser->name.' le '.$this->selectedDate.$slot.' a été créé !');
        if (auth()->user()->role == 0) {
    ...
    public function sendAppointmentNotification($appointmentData)
    {
        ...
        Mail::to('cura00229@gmail.com')->send(new AppointmentUpdated($appointmentData));
        ...
        return 'Les notifications de rendez-vous ont été envoyées avec succès !';
    }
